...
class AppStore extends Component {
  ...
  getSearchResults = () => {
    ...
    const filteredSearchInput = appsList.filter(eachApp =>
      eachApp.appName.toLowerCase().includes(givenInput.toLowerCase()),
    )
    return filteredSearchInput
  }

  getAppsList = filteredSearchApps => {
    const {isActiveTab} = this.state

    const filteredAppsList = filteredSearchApps.filter(
  ...
  render() {
    const {isActiveTab, givenInput} = this.state

    const filteredSearchApps = this.getSearchResults()

    const filteredApps = this.getAppsList(filteredSearchApps)

    return (
      <div className="app-container">
        <div className="heading-and-search-input-container">
          <h1 className="app-store-heading">App Store</h1>
          <div className="tabs-and-apps-container">
            <div className="search-container">
              <input
                ...
                placeholder="Search"
                value={givenInput}
                onChange={this.onChangeInputDetails}
              />
            ...
            <ul className="apps-container">
              {filteredApps.map(eachApp => (
                <AppItem key={eachApp.id} eachAppDetails={eachApp} />
              ))}
            </ul>
          </div>
        </div>
